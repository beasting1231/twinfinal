...
// Comprehensive list of all countries with their calling codes
const countries = [
...
export function CountryCodeSelect({ value, onChange, lightMode = false }: CountryCodeSelectProps) {
  ...
  return (
    <div ref={containerRef} className="relative">
      <button
        ...
        className={`flex items-center gap-1 h-10 px-3 rounded-md transition-colors ${
          lightMode
      ...
      {isOpen && (
        <div className={`absolute z-50 top-full mt-1 w-80 rounded-lg shadow-xl max-h-96 overflow-hidden ${
          lightMode
            ? "!bg-white !border !border-gray-200"
            : "bg-zinc-800 border border-zinc-700"
        }`}>
          {/* Search Input */}
          <div className={`p-2 border-b ${lightMode ? "border-gray-200" : "border-zinc-700"}`}>
            <div className="relative">
              <Search className={`absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 ${lightMode ? "text-gray-400" : "text-zinc-400"}`} />
              <input
                ref={searchInputRef}
                type="text"
                value={searchQuery}
                ...
                className={`w-full pl-10 pr-3 py-2 rounded-md text-sm focus:outline-none focus:ring-2 focus:ring-gray-400 ${
                  lightMode
          ...
          <div className="overflow-y-auto max-h-80">
            {popularFiltered.length > 0 && (
              <>
                <div className={`px-3 py-1.5 text-xs font-medium ${
                  lightMode ? "!text-gray-500 !bg-gray-50" : "text-zinc-400 bg-zinc-900"
                }`}>
                  Popular
                </div>
                {popularFiltered.map((country) => (
                  <button
                    ...
                    className={`w-full px-3 py-2 text-left transition-colors flex items-center justify-between ${
                      lightMode
                        ? `hover:!bg-gray-100 ${selectedCountry.code === country.code ? "!bg-gray-100" : ""}`
                        : `hover:bg-zinc-700 ${selectedCountry.code === country.code ? "bg-zinc-700" : ""}`
                    }`}
                  >
                    <span className={lightMode ? "!text-gray-900" : "text-white"}>{country.name}</span>
                    <span className={`text-sm ${lightMode ? "!text-gray-500" : "text-zinc-400"}`}>{country.dialCode}</span>
                  </button>
                ))}
              </>
            )}
              ...
              <>
                {popularFiltered.length > 0 && (
                  <div className={`px-3 py-1.5 text-xs font-medium border-t ${
                    lightMode ? "!text-gray-500 !bg-gray-50 !border-gray-200" : "text-zinc-400 bg-zinc-900 border-zinc-700"
                  }`}>
                    All Countries
                  </div>
                )}
            ...
            {filteredCountries.length === 0 && (
              <div className={`px-3 py-4 text-center text-sm ${lightMode ? "!text-gray-500" : "text-zinc-500"}`}>
                No countries found
              </div>
            )}
          </div>
